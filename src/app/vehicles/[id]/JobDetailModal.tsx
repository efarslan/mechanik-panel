...
const allQuickJobNames: string[] = Array.from(
  new Set(Object.values(quickJobs).flat().filter(Boolean)),
);
...
export default function JobDetailModal({
  ...
  const [editForm, setEditForm] = useState({ title: "", notes: "", laborFee: "" });
  ...
  useEffect(() => {
    if (!job) {
      setIsEditMode(false);
      setEditForm({ title: "", notes: "", laborFee: "" });
      setItems([]);
      setImages([]);
      setNewFiles([]);
      setFileError(null);
    ...
    setEditForm({
      title: job.title ?? "",
      notes: job.notes ?? "",
      laborFee: job.laborFee != null && job.laborFee !== 0
        ? String(job.laborFee)
        : "",
    });
  ...
  // ── Computed totals ──────────────────────────────────────────────────────
  const laborFee = Number(job.laborFee) || 0;
  const partsTotal = items.reduce((sum, item) => sum + (item.quantity || 0) * (item.unitPrice || 0), 0);
  const grandTotal = partsTotal + laborFee;

  // Edit form computed
  const editLaborFee = Number(editForm.laborFee) || 0;
  const editPartsTotal = items.reduce((sum, item) => sum + (item.quantity || 0) * (item.unitPrice || 0), 0);
  const editGrandTotal = editPartsTotal + editLaborFee;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload: Partial<JobListItem> & { updatedAt: Date; laborFee: number } = {
        title: trimmedTitle,
        ...
        <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-sm border-b border-gray-100 px-5 pt-4 pb-3 rounded-t-3xl sm:rounded-t-2xl">
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <button
              type="button"
              onClick={handleClose}
              className="shrink-0 w-7 h-7 inline-flex items-center justify-center rounded-full bg-gray-100 text-gray-500 hover:bg-gray-200 hover:text-gray-900 transition text-sm"
              aria-label="Kapat"
            >
              ✕
            </button>
          ...
          {!isCompleted ? (
            <div className="mt-3 flex items-center gap-2 flex-wrap">
              ...
              >
                ...
                {isEditMode ? "Düzenlemeyi Kapat" : "Düzenle / Güncelle"}
              </button>

              {/* Separator */}
              <span className="w-px h-5 bg-gray-200" />

              {/* Delete / Delete confirm block */}
              {!isCancelled ? (
                !showDeleteConfirm ? (
                  <button
                    type="button"
              ...
              {/* Tamamlandı Olarak İşaretle section */}
              {!showCompleteConfirm ? (
                <button
                  type="button"
          ...
          {(items.length > 0 || laborFee > 0) && (
            <section className="space-y-2">
              ...
              <div className="border border-gray-200 rounded-xl overflow-hidden">
                ...
                <div className="divide-y divide-gray-100">
                  {items.map((item, index) => {
                    ...
                    return (
                      <div
                        key={`${item.name}-${index}`}
                        className="grid grid-cols-[1.5fr_1fr_auto_auto_auto] gap-3 px-4 py-2.5 text-xs items-center"
                      >
                        ...
                        <span className="text-right text-gray-600">
                          {item.unitPrice ? `${item.unitPrice.toLocaleString("tr-TR")} ₺` : "—"}
                        </span>
                        <span className="text-right font-semibold text-gray-900">
                          {lineTotal ? `${lineTotal.toLocaleString("tr-TR")} ₺` : "—"}
                        </span>
                      </div>
                    );
                  })}
                ...
                {/* Labor fee row */}
                {laborFee > 0 && (
                  <div className="grid grid-cols-[1fr_auto] gap-3 px-4 py-2.5 border-t border-gray-100 bg-amber-50/50 items-center">
                    <div className="flex items-center gap-2">
                      <svg className="w-3.5 h-3.5 text-amber-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M11.42 15.17L17.25 21A2.652 2.652 0 0021 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 11-3.586-3.586l5.654-4.654m5.896-5.896l3.83-3.83a2.652 2.652 0 00-3.755-3.755L11.42 7.667" />
                      </svg>
                      <span className="text-xs font-medium text-amber-700">İşçilik Ücreti</span>
                    </div>
                    <span className="text-right text-xs font-semibold text-amber-700">
                      {laborFee.toLocaleString("tr-TR")} ₺
                    </span>
                  </div>
                )}
                ...
                <div className="border-t border-gray-200 bg-gray-50 px-4 py-2.5 flex items-center justify-between">
                  <span className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Genel Toplam</span>
          ...
          {isEditMode && !isCompleted && (
            <section className="space-y-4 border-t border-dashed border-gray-200 pt-5">
              ...
              <div className="space-y-3">
                ...
                {/* Labor fee */}
                <div className="border border-amber-100 bg-amber-50/50 rounded-xl px-4 py-3 flex items-center gap-4">
                  <div className="flex items-center gap-2 shrink-0">
                    <svg className="w-4 h-4 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M11.42 15.17L17.25 21A2.652 2.652 0 0021 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 11-3.586-3.586l5.654-4.654m5.896-5.896l3.83-3.83a2.652 2.652 0 00-3.755-3.755L11.42 7.667" />
                    </svg>
                    <label className="text-xs font-semibold text-amber-700 whitespace-nowrap">İşçilik Ücreti</label>
                  </div>
                  <div className="flex-1 flex items-center gap-2 justify-end">
                    <input
                      ...
                      min={0}
                      placeholder="0"
                      value={editForm.laborFee}
                      onChange={(e) => setEditForm((prev) => ({ ...prev, laborFee: e.target.value }))}
                      className="w-32 border border-amber-200 bg-white rounded-lg px-3 py-1.5 text-sm text-right focus:outline-none focus:ring-2 focus:ring-amber-300 focus:border-amber-300 transition"
                    />
                    <span className="text-sm font-semibold text-amber-700">₺</span>
                  </div>
                </div>

                {/* Edit grand total preview */}
                {(items.length > 0 || editLaborFee > 0) && (
                  <div className="flex items-center justify-between px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl">
                    <span className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Genel Toplam</span>
                    <span className="text-sm font-bold text-gray-900">
                      {editGrandTotal.toLocaleString("tr-TR")} ₺
                    </span>
                  </div>
                )}
